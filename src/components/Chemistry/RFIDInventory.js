...
class RFIDInventory extends Component {
  ...
  findContainer = async barcode => {
    if(!(this.state.containers.map(c=>c.barcode).includes(barcode))) {
      let container = await this.props.query(barcode);
      if (container) {
        this.setState({
          containers: this.state.containers.concat(container),
        });
      }
    }
  }
  ...
  componentDidMount() {
    const ws = new WebSocket('ws://10.125.208.218:8080');
    ...
    ws.onmessage = async event => {
      ...
      const barcode = hex_to_ascii(event.data);
      await this.findContainer(barcode);
    };
    this.findContainer(hex_to_ascii('74657374303030'));
  }
